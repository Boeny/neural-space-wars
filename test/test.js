...
var Vector2 = function () {
	...
	_createClass(Vector2, [{
		key: 'roundSelf',
		value: function roundSelf(depth) {
			return this.withSelf(function (coo) {
				return Vector2.round(coo, depth);
			});
		}
		...
		key: 'withSelf',
		value: function withSelf() {
			var forCoo = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : function () {};
		...
		key: 'inRange',
		value: function inRange(v, radius) {
			return Vector2.isNumInRange(this.x, v.x - radius, v.x + radius) && Vector2.isNumInRange(this.y, v.y - radius, v.y + radius);
		}
	}, {
		key: 'equals',
		value: function equals(v, depth) {
			if (depth !== undefined) {
				return Vector2.roundVector(this, depth).equals(Vector2.roundVector(v, depth));
			}
		...
		key: 'setAngle',
		value: function setAngle(newAngle, center) {
		...
		value: function round(n) {
			var depth = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1000;

			return Math.round(depth * n) / depth;
		}
	}, {
		key: 'roundVector',
		value: function roundVector(v) {
			var depth = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1000;
		...
		value: function check(v) {
			...
			if ((typeof v === 'undefined' ? 'undefined' : _typeof(v)) != 'object') return new Vector2(v, v);
			return v;
		}
	}, {
		key: 'isNumInRange',
		value: function isNumInRange(n, min, max) {
			return n > min && n < max;
		}
	}, {
		key: 'zero',
...
__webpack_require__(21)({
	'ship': {
		'must shift to the right if pos = (0,0), angle = 0 and velocity = (1,0)': function mustShiftToTheRightIfPos00Angle0AndVelocity10() {
			var objects = _canvas2.default.createObjects([__webpack_require__(46)]);
			_canvas2.default.setObjectsEnv(objects);

			var ship = objects.ship;
			ship.position = _vector2.default.zero;
			ship.rotation = 0;
			ship.acceleration = _vector2.default.up;
			ship.incVelocity();
			ship.Update();

			return [ship.position, _vector2.default.right];
		}
	}
});
...
_canvas2.default.createObjects = function (models) {
	if (!models) return;
	...
	if (models instanceof Array) {
		for (var i = 0; i < models.length; i++) {
			var obj = models[i]; // {className: class}
			var className = obj_key(obj);
			objects[className.toLowerCase()] = new obj[className]();
		}
	} else {
		for (var name in models) {
			var _obj = models[name]; // {className: class}
			objects[name] = new _obj[obj_key(_obj)]();
		}
	}
...
_canvas2.default.setObjectsEnv = function (objects, multiplier, updates) {
	var renderer = new _canvas2.default(multiplier, !!updates);
	var controls = new _controls2.default(renderer);
	...
	var _loop = function _loop() {
		...
		if (updates && (obj.render || obj.Update)) {
			...
			if (obj.render && obj.Update) upd = function upd() {
				obj.Update();
				obj.render();
			};
	...
	return { renderer: renderer, controls: controls };
};
 ...
 */
_canvas2.default.ready = function (models, multiplier) {
	var objects = _canvas2.default.createObjects(models);

	global.onload = function () {
		var updates = [];

		var _Canvas$setObjectsEnv = _canvas2.default.setObjectsEnv(objects, multiplier, updates),
		    renderer = _Canvas$setObjectsEnv.renderer,
		    controls = _Canvas$setObjectsEnv.controls;
...
var Canvas = function () {
	...
	_createClass(Canvas, [{
		...
		key: 'createData',
		value: function createData(w, h) {
			return this.canvas.createImageData(w || this.width, h || this.height);
		}
	}, {
		key: 'renderData',
		value: function renderData(img, x, y) {
		...
		key: 'getColorAtIndex',
		value: function getColorAtIndex(img, i) {
			return {
				r: img.data[i] || 0,
				g: img.data[i + 1] || 0,
				b: img.data[i + 2] || 0,
		...
		key: 'setColorAtIndex',
		value: function setColorAtIndex(img, i, c) {
			c = c || {};
		...
		key: 'getColorAt',
		value: function getColorAt(img, x, y) {
			return this.getColorAtIndex(img, 4 * (x + y * this.DOM.width));
		}
	}, {
		key: 'setColorAt',
		value: function setColorAt(p, x, y, c) {
		...
		value: function setColor(color) {
			if (!color) return;
			if ((typeof color === 'undefined' ? 'undefined' : _typeof(color)) != 'object') color = { fill: color, stroke: color };
		...
		key: 'drawLine',
		value: function drawLine(p1, p2, width) {
		...
		key: 'drawLineTo',
		value: function drawLineTo(p, init, width) {
		...
		key: 'drawLines',
		value: function drawLines(points, width) {
		...
		key: 'drawPoly',
		value: function drawPoly(points, width) {
			this.drawLines(points, width);
			this.drawLineTo(points[0], null, width); // close path
		}
	}, {
		key: 'drawCircle',
		value: function drawCircle(p, radius, width) {
			this.canvas.beginPath();
		...
		key: 'drawText',
		value: function drawText(text, p, font) {
...
var Controls = function () {
	...
	_createClass(Controls, [{
		...
		value: function Bind(objects) {
			...
			var _loop = function _loop() {
				...
				var _loop2 = function _loop2(eventName) {
					var handlerName = _this.handlers[eventName];

					if (obj[handlerName]) {
						document.body.addEventListener(eventName, function (e) {
							if (!_this.enabled) return;
							_this.keys.CTRL = e.ctrlKey;
							_this.eventHandler(e, obj, handlerName);
						}, false);
					}
				};

				for (var eventName in _this.handlers) {
					_loop2(eventName);
				}
			};
			...
			document.addEventListener('pointerlockchange', function (e) {
				return _this.onPointerLockingChange(e);
			}, false);
			...
			document.addEventListener('pointerlockerror', function (e) {
				return _this.onPointerLockingError(e);
			}, false);
			document.addEventListener('mozpointerlockerror', function (e) {
				return _this.onPointerLockingError(e);
			}, false);
			document.addEventListener('webkitpointerlockerror', function (e) {
				return _this.onPointerLockingError(e);
			}, false);
		}
	}, {
		key: 'eventHandler',
		value: function eventHandler(e, obj, handlerName) {
			...
			switch (handlerName) {
				...
				case 'onMouseMove':
					...
					return handler(new _vector2.default(e.clientX, e.clientY), new _vector2.default(e.movementX || e.mozMovementX || e.webkitMovementX || 0, e.movementY || e.mozMovementY || e.webkitMovementY || 0));
				...
				case 'onMouseDown':
					if (!this.enableMouse) return;
					return handler(e.button, new _vector2.default(e.clientX, e.clientY));

				case 'onTouchStart':
				case 'onTouchMove':
				case 'onTouchEnd':
					if (!this.enableTouch) return;
					var points = new Array(e.touches.length);

					for (var i = 0; i < points.length; i++) {
						points[i] = new _vector2.default(e.touches[i].pageX, e.touches[i].pageY);
					}
		...
		key: 'onPointerLockingError',
		value: function onPointerLockingError() {}
	}, {
		key: 'onPointerLockingChange',
		value: function onPointerLockingChange() {}

		// Ask the browser to lock the pointer

	}, {
		key: 'lockPointer',
		value: function lockPointer(elem) {
			var lock = elem.requestPointerLock || elem.mozRequestPointerLock || elem.webkitRequestPointerLock;
			lock && lock();
		}
	}, {
		key: 'isPointerLocked',
		value: function isPointerLocked(elem) {
...
function run_test(text, callback) {
	it(text, function () {
		...
		if (res instanceof Array) {
			assert[res[0] && _typeof(res[0]) == 'object' ? 'deepEqual' : 'equal'](res[0], res[1], res[2]);
			return;
		}
...
module.exports = function (modules) {
	if (typeof obj_val(obj_val(modules)) == 'function') {
		// tests -> subtests -> function
		test(modules);
		return;
	}
...
var pathval = __webpack_require__(24);

/*!
 * test utility
 */

exports.test = __webpack_require__(25);
...
exports.compareByInspect = __webpack_require__(37);

/*!
 * Get own enumerable property symbols method
 */

exports.getOwnEnumerablePropertySymbols = __webpack_require__(16);

/*!
 * Get own enumerable properties method
 */

exports.getOwnEnumerableProperties = __webpack_require__(38);

/*!
 * Checks error against a given set of criteria
 */

exports.checkError = __webpack_require__(39);
...
exports.isNaN = __webpack_require__(40);
...
var getOwnEnumerablePropertySymbols = __webpack_require__(16);
...
var _figure = __webpack_require__(47);
...
var Ship = exports.Ship = function () {
	...
	function Ship() {
		...
		this.figure = new _figure2.default([0, 10], [-5, -10], [5, -10]);
		this.radius = 10; // for collisions

		this.velocity = new _vector2.default();
		this.acceleration = _vector2.default.up.mult(0.1);
		this.angularVelocity = PI / 40;

		this.isDestroyed = false;
	}

	// after renderer has created but before it has added to the scene


	_createClass(Ship, [{
		key: 'Start',
		value: function Start(objects) {
			this.controls.enabled = true;

			this.position = new _vector2.default(this.renderer.width / 2 - 100, this.renderer.height / 2);
			this.rotation = -PI;
		...
		value: function render() {
			...
			this.renderer.drawPoly(this.figure.updatePoints(this.position, this.rotation));
		}
		...
		key: 'onTouchStart',
		value: function onTouchStart(points) {
			this.moveTo(points[0]);
		}
	}, {
		key: 'onTouchMove',
		value: function onTouchMove(points) {
			this.moveTo(points[0]);
		}
	}, {
		key: 'onTouchEnd',
...
var Figure = function () {
	...
	_createClass(Figure, [{
		key: 'setOffsets',
		value: function setOffsets(points) {
			this.offsets = [];
			var PI_2 = Math.PI / 2;

			for (var i = 0; i < points.length; i++) {
				var pos = _vector2.default.check(points[i]);
		...
		key: 'updatePoints',
		value: function updatePoints(pos, rot) {
		...
		key: 'setPoints',
		value: function setPoints(pos, rot) {
		...
		key: 'setPoint',
		value: function setPoint(offset, pos, rot) {
			var p = offset.position.clone();
